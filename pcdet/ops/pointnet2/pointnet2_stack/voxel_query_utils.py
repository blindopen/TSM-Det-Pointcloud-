...
class VoxelQuery(Function):
    ...
    def forward(ctx, max_range: int, radius: float, nsample: int, xyz: torch.Tensor, \
        ...
        cnt_unique = torch.cuda.IntTensor(M, 1).zero_()

        z_range, y_range, x_range = max_range
        pointnet2.voxel_query_wrapper(M, Z, Y, X, nsample, radius, z_range, y_range, x_range, \
                    new_xyz, xyz, new_coords, point_indices, idx, cnt_unique)

        empty_ball_mask = (idx[:, 0] == -1)
        idx[empty_ball_mask] = 0
        volume = (x_range*2+1)*(y_range*2+1)*(z_range*2+1)
        density = cnt_unique/volume
        density_score = 1-density

        return idx, empty_ball_mask, density
...
class VoxelQueryAndGrouping(nn.Module):
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        idx1, empty_ball_mask1, density_score = voxel_query(self.max_range, self.radius, self.nsample, xyz, new_xyz, new_coords, voxel2point_indices)
        ...
        # count = 0
        # start_idx = 0
        # for i, val in enumerate(new_xyz_batch_cnt):
        #     idx1[start_idx:start_idx + val] -= count
        ...
        grouped_xyz = pointnet2_utils.grouping_operation(xyz, xyz_batch_cnt, idx1, new_xyz_batch_cnt)
        # grouped_features: (M1 + M2, C, nsample)
        grouped_features = pointnet2_utils.grouping_operation(features, xyz_batch_cnt, idx1, new_xyz_batch_cnt)
        
        return grouped_features, grouped_xyz, empty_ball_mask1, density_score


class VoxelQueryDilated(Function):

    @staticmethod
    def forward(ctx, max_range: int, stride: int, former_radius: float, radius: float, nsample: int, xyz: torch.Tensor, \
                new_xyz: torch.Tensor, new_coords: torch.Tensor, point_indices: torch.Tensor):
        """
        Args:
            ctx:
        ...
        idx = torch.cuda.IntTensor(M, nsample).zero_()
        cnt_unique = torch.cuda.IntTensor(M, 1).zero_()
        idx_cnt = torch.cuda.IntTensor(M, 1).zero_()

        z_range, y_range, x_range = max_range
        z_stride, y_stride, x_stride = stride
        pointnet2.voxel_query_dilated_wrapper(M, Z, Y, X, nsample, former_radius, radius,
                                              z_range, y_range, x_range,
                                              z_stride, y_stride, x_stride,
                                              new_xyz, xyz, new_coords, point_indices, idx, cnt_unique, idx_cnt)

        empty_ball_mask = (idx[:, 0] == -1)
        idx[empty_ball_mask] = 0
        # volume = (x_range * 2 + 1) * (y_range * 2 + 1) * (z_range * 2 + 1)
        density = cnt_unique / nsample
        density_score = torch.clamp(density, max=1.0)
        # density_score = 1 - density

        return idx, empty_ball_mask, density_score
...
voxel_query_dilated = VoxelQueryDilated.apply


class VoxelQueryAndGroupingDilated(nn.Module):
    def __init__(self, max_range: int, stride: int, former_radius: float, radius: float, nsample: int):
        """
        Args:
            radius: float, radius of ball
            nsample: int, maximum number of features to gather in the ball
        """
        super().__init__()
        self.max_range, self.stride, self.former_radius, self.radius, self.nsample = \
            max_range, stride, former_radius, radius, nsample
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        # print(delta_print)
        # idx: (M1 + M2 ..., nsample), empty_ball_mask: (M1 + M2 ...)
        idx1, empty_ball_mask1, density_score = voxel_query_dilated(
            self.max_range, self.stride, self.former_radius, self.radius, self.nsample,
            xyz, new_xyz, new_coords, voxel2point_indices)
        ...
        # xyz_features = torch.cat([xyz, features], dim=-1)
        # grouped_xyzfeatures = pointnet2_utils.grouping_operation(xyz_features, xyz_batch_cnt, idx, new_xyz_batch_cnt)
