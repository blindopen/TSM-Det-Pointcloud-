...
__global__ void voxel_query_kernel_stack(int M, int R1, int R2, int R3, int nsample, 
            ...
            const float *xyz, const int *new_coords, const int *point_indices, int *idx, int *cnt_unique) {
...
void voxel_query_kernel_launcher_stack(int M, int R1, int R2, int R3, int nsample,
    ...
    voxel_query_kernel_stack<<<blocks, threads>>>(M, R1, R2, R3, nsample, radius, z_range, y_range, x_range, new_xyz, xyz, new_coords, point_indices, idx, cnt_unique);
...
__global__ void voxel_query_dilated_kernel_stack(int M, int R1, int R2, int R3, int nsample,
            float former_radius, float radius, int z_range, int y_range, int x_range,int z_stride, int y_stride, int x_stride, const float *new_xyz,
            const float *xyz, const int *new_coords, const int *point_indices, int *idx, int *cnt_unique, int *idx_cnt) {
    // :param new_coords: (M1 + M2 ..., 4) centers of the ball query
    // :param point_indices: (B, Z, Y, X)
    // output:
    //      idx: (M1 + M2, nsample)
    ...
    float radius2 = radius * radius;
    float former_radius2 = former_radius * former_radius;
    float new_x = new_xyz[0];
    float new_y = new_xyz[1];
    float new_z = new_xyz[2];
    ...
    int range_point_num = 0;
    // for (int dz = -1*z_range; dz <= z_range; ++dz) {
    for (int dz = -1*z_range; dz <= z_range; dz = dz+z_stride) {
        int z_coord = new_coords_z + dz;
        if (z_coord < 0 || z_coord >= R1) continue;

        for (int dy = -1*y_range; dy <= y_range; dy = dy+y_stride) {
            int y_coord = new_coords_y + dy;
            if (y_coord < 0 || y_coord >= R2) continue;

            for (int dx = -1*x_range; dx <= x_range; dx = dx+x_stride) {
                int x_coord = new_coords_x + dx;
                if (x_coord < 0 || x_coord >= R3) continue;
                ...
                if (dist2 > radius2 || dist2 < former_radius2) continue;
...
void voxel_query_dilated_kernel_launcher_stack(int M, int R1, int R2, int R3, int nsample,
    float former_radius, float radius, int z_range, int y_range, int x_range, int z_stride, int y_stride, int x_stride, const float *new_xyz,
    const float *xyz, const int *new_coords, const int *point_indices, int *idx, int *cnt_unique, int *idx_cnt) {
    // :param new_coords: (M1 + M2 ..., 4) centers of the voxel query
    // :param point_indices: (B, Z, Y, X)
    ...
    voxel_query_dilated_kernel_stack<<<blocks, threads>>>(M, R1, R2, R3, nsample, former_radius, radius, z_range, y_range, x_range, z_stride, y_stride, x_stride, new_xyz, xyz, new_coords, point_indices, idx, cnt_unique, idx_cnt);
    // cudaDeviceSynchronize();  // for using printf in kernel function
