...
def mask_points_by_range(points, limit_range):
    mask = (points[:, 0] >= limit_range[0]) & (points[:, 0] <= (limit_range[3]-0.0002)) \
           & (points[:, 1] >= limit_range[1]) & (points[:, 1] <= (limit_range[4]-0.0002)) \
           & (points[:, 2] >= limit_range[2]) & (points[:, 2] <=(limit_range[5]-0.0002))
    return mask
...
def get_voxel_centers_tensor(voxel_coords, voxel_size, point_cloud_range):
    """
    Args:
        voxel_coords: (N, 3)
        downsample_times:
    ...
    assert voxel_coords.shape[1] == 3
    voxel_centers = voxel_coords[:, [2, 1, 0]].float()  # (xyz)
    voxel_centers = (voxel_centers + 0.5) * voxel_size + point_cloud_range[0:3]
    return voxel_centers
