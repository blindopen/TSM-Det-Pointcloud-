...
def get_overlapping_voxel_indices(point_coords, downsample_times, voxel_size, point_cloud_range):
    ...
    # downsample_times = torch.tensor(downsample_times, device=point_coords.device)
...
def get_voxel_indices(point_coords, voxel_size, point_cloud_range):
    """
    Args:
        point_coords: (N, 3)
        downsample_times: (int)
    ...
    # voxel_size = torch.tensor(voxel_size, device=point_coords.device).float()

    # voxel_size = voxel_size * downsample_times

    # pc_range = torch.tensor(point_cloud_range, device=point_coords.device).float()
    # delta = time.time() - start_time
    # delta_print = 'Generate label finished(sec_per_forward_example: %.4f second).' % delta
    # print(delta_print)
    voxel_indices = ((point_coords - point_cloud_range[0:3]) / voxel_size)
...
def get_nonempty_voxel_feature_indices(voxel_indices, x_conv):
    ...
    x_conv_hash_table = common_utils.generate_voxel2pinds(x_conv)
    # voxel_indices_max = torch.max(x_conv_hash_table.view(-1), dim=0)[0]
    # print(voxel_indices_max)
    ...
    overlapping_voxel_feature_nonempty_mask = overlapping_voxel_feature_indices != -1
    # overlapping_voxel_feature_indices_min = torch.min(overlapping_voxel_feature_indices, dim=0)[0]
    # print(overlapping_voxel_feature_indices_min)

    # Filter and shift indices back by -1
    overlapping_voxel_feature_indices_nonempty = overlapping_voxel_feature_indices[overlapping_voxel_feature_nonempty_mask]
    return overlapping_voxel_feature_indices_nonempty.long(), overlapping_voxel_feature_nonempty_mask
...
def get_centroid_per_voxel(points, voxel_idxs, num_points_in_voxel=None):
    ...
    centroid_voxel_idxs, unique_idxs_ori, labels_count = voxel_idxs.unique(dim=0, return_inverse=True, return_counts=True)
    # start_time = time.time()
    unique_idxs = unique_idxs_ori.view(unique_idxs_ori.size(0), 1).expand(-1, points.size(-1))
    # delta = time.time() - start_time
    # delta_print = 'Generate label finished(sec_per_forward_example: %.4f second).' % delta
    # print(delta_print)
    ...
    return centroids, centroid_voxel_idxs, labels_count, unique_idxs_ori
