...
class UNetV2(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['raw_points_bxyz'] = torch.cat((x_up1.indices[:, 0:1].float(), point_coords), dim=1)
        return batch_dict


class UNetV2Backbone(nn.Module):
    """
    Sparse Convolution based UNet for point-wise feature learning.
    Reference Paper: https://arxiv.org/abs/1907.03670 (Shaoshuai Shi, et. al)
    From Points to Parts: 3D Object Detection from Point Cloud with Part-aware and Part-aggregation Network
    ...
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        ...
        self.conv_scale4 = spconv.SparseSequential(
            # [400, 352, 11] <- [200, 176, 5]
            block(128, 64, 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1), indice_key='spconv4', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4'),
        )
        self.conv_scale5 = spconv.SparseSequential(
            # [200, 176, 5] <- [100, 88, 5]
            block(128, 64, 3, norm_fn=norm_fn, stride=(1, 2, 2), padding=1, indice_key='spconv5', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm5'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm5'),
        )
        self.num_point_features = 16
    ...
    def forward(self, batch_dict):
        ...
        x_up2 = self.UR_block_forward(x_conv2, x_up3, self.conv_up_t2, self.conv_up_m2, self.inv_conv2)
        # [1600, 1408, 41] <- [1600, 1408, 41]
        x_up1 = self.UR_block_forward(x_conv1, x_up2, self.conv_up_t1, self.conv_up_m1, self.conv6)

        x_scale1 = self.conv_scale1(x_up1)
        x_scale1_temp = replace_feature(x_scale1, torch.cat((x_scale1.features, x_up2.features), dim=1))
        x_scale2 = self.conv_scale2(x_scale1_temp)
        x_scale2_temp = replace_feature(x_scale2, torch.cat((x_scale2.features, x_up3.features), dim=1))
        x_scale3 = self.conv_scale3(x_scale2_temp)
        x_scale3_temp = replace_feature(x_scale3, torch.cat((x_scale3.features, x_up4.features), dim=1))
        x_scale4 = self.conv_scale4(x_scale3_temp)
        x_scale4_temp = replace_feature(x_scale4, torch.cat((x_scale4.features, x_up5.features), dim=1))
        x_scale5 = self.conv_scale5(x_scale4_temp)
