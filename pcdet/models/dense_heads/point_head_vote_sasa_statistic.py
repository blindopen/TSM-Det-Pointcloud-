...
class PointHeadVoteSASAStatistic(PointHeadTemplate):
    """
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        # spconv_mlps = self.vsa_cfg.SPCONV_MLPS_PRE.copy()
        # spconv_mlps = [256] + spconv_mlps
        ...
        self.VSA_module = pointnet2_modules.VoxelPointnetSAModuleFSMSG(
    ...
    def forward(self, batch_dict):
        ...
        if self.training and (
            ...
            for i in range(self.num_class):
                ...
                if len(cur_class_features) > 0:
                    ...
                    with torch.no_grad():
                        delta_statistic = max_cur_class_features - self.object_mean[i, :]
                        self.object_momentum[i] = 0.90 * self.object_momentum[i] + delta_statistic
                        self.object_statistic_features[i, :] = self.object_statistic_features[i, :] + self.object_momentum[i]
                        self.object_mean[i] = max_cur_class_features
        ...
        # start_iter = 0
        # point_features_for_statistic = point_features.permute(0, 2, 1).contiguous().view(-1, point_features.shape[1])
        # if self.training and (batch_dict['accumulated_iter'] is not None and batch_dict['accumulated_iter'] >= start_iter):
        #     for i in range(3):
        #         cur_class_mask = idx_cls == i
        #         cur_score_norm = score_cls[cur_class_mask]
        #         filter_score_mask = cur_score_norm >= 0.3
        ...
        statistic_extended_for_reg = point_features.new_zeros(len(score_cls), self.object_statistic_features.shape[-1])
        for i in range(self.num_class):
            cur_class_mask = idx_cls == i
        ...
        if self.training:
            ...
            # ret_dict['l_new_centroids_masked'] = batch_dict['l_new_centroids_masked']
            # ret_dict['l_sp_tensor_features_selected'] = batch_dict['l_sp_tensor_features_selected']
            ...
            if self.enable_sasa:
                ...
                ret_dict.update({
                    ...
                    # 'point_sasa_part_preds': batch_dict['point_part_scores_list']
                })
