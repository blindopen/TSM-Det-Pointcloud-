...
class DSASNetHead(PointHeadTemplate):
    """
    ...
    def __init__(self, voxel_size, point_cloud_range, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        self.point_feature_layers = self.make_fc_layers_v1(
            fc_cfg=self.model_cfg.FEATURES_FC,
            input_channels=64,
            output_channels=128,
            end_bn_active=True,
        )
        self.pos_encode_layers = self.make_fc_layers_v1(
            fc_cfg=self.model_cfg.POS_FC,
            input_channels=3,
            output_channels=256,
            end_bn_active=True,
        )
        self.encoded_feature_layers = self.make_fc_layers_v1(
            fc_cfg=self.model_cfg.EF_FC,
            input_channels=256,
            output_channels=128,
            end_bn_active=True,
        )
        self.ef_attention_layers = self.make_fc_layers_v1(
            fc_cfg=self.model_cfg.EFA_FC,
            input_channels=256,
            output_channels=1,
        )
        self.pf_attention_layers = self.make_fc_layers_v1(
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['multi_scale_3d_features']['x_points_mean'] = \
            batch_dict['multi_scale_3d_features']['x_points_mean'].replace_feature(semantic_geo_features)
        batch_dict['multi_scale_3d_features']['x_points_max'] = \
            batch_dict['multi_scale_3d_features']['x_points_max'].replace_feature(semantic_geo_features)
