...
class PointHeadVoteSASA(PointHeadTemplate):
    ...
    def get_box_layer_loss(self, tb_dict=None):
        ...
        if reg_weights.sum() > 0:
            ...
            if self.model_cfg.LOSS_CONFIG.get('RDIOU_REGRESS_REGULARIZATION', False):
                ...
                point_loss_iou = 1 - rdiou.squeeze(0)
    ...
    def get_similarity_loss(self, tb_dict=None):
        ...
        for i in range(len(l_new_centroids_masked)):
            if l_new_centroids_masked[i] is not None:
                ...
                # l1_loss = torch.abs(l_new_centroids_masked[i]-l_sp_tensor_features_selected[i])
    ...
    def get_loss(self, tb_dict=None):
        ...
        point_loss_similarity, tb_dict_3 = self.get_similarity_loss()
        ...
        point_loss = point_loss_vote + point_loss_cls + point_loss_box+ point_loss_similarity
    ...
    def forward(self, batch_dict):
        ...
        _, point_features, _, _, _, _, _, l_new_centroids_masked, l_sp_tensor_features_selected = self.VSA_module(
            xyz=point_coords,
            new_xyz=vote_coords,
        ...
        # score_mask = torch.sigmoid(point_cls_preds)>=0.1
        # num_score_pos = torch.sum(score_mask.float())
        # if num_score_pos > 0:
        #     cls_aware_features = point_features.permute(0, 2, 1).contiguous()
        #     cls_aware_features = cls_aware_features.view(-1, cls_aware_features.shape[-1]).contiguous()
        #     score_mask = score_mask.view(-1)
        #     cls_aware_features_max = torch.max(cls_aware_features[score_mask], dim=0)[0]
        #     cls_aware_features_mean = torch.mean(cls_aware_features[score_mask], dim=0)[0]
        #     temp = (cls_aware_features_max-cls_aware_features_mean).view(1, -1, 1)
        # else:
        #     temp = 0.0
