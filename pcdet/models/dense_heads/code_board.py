...
class PointHeadVoteSASAStatistic(PointHeadTemplate):
    ...
    def get_cls_layer_loss(self, tb_dict=None):
        ...
        if 'WithCenterness' in loss_cfgs.LOSS_CLS:
            ...
            u, rdiou = self.get_rdiou(
                point_box_preds[positives, :].view(1, -1, 7),
                point_box_labels[positives, :].view(1, -1, 7))
    ...
    def get_box_layer_loss(self, tb_dict=None):
        ...
        if reg_weights.sum() > 0:
            ...
            if self.model_cfg.LOSS_CONFIG.get('RDIOU_REGRESS_REGULARIZATION', False):
                ...
                u, rdiou = self.get_rdiou(
                    point_box_preds[pos_mask, :].view(1, -1, 7),
                    point_box_labels[pos_mask, :].view(1, -1, 7))
    ...
    def get_similarity_loss(self, tb_dict=None):
        ...
        for i in range(len(l_new_centroids_masked)):
            if l_new_centroids_masked[i] is not None:
                cur_loss = torch.mean(
                    1 - F.cosine_similarity(l_new_centroids_masked[i], l_sp_tensor_features_selected[i], dim=-1))
                l1_loss = torch.abs(
                    l_new_centroids_masked[i] - l_sp_tensor_features_selected[i] * l_sp_tensor_scale_selected[i])
    ...
    def forward(self, batch_dict):
        ...
        start_iter = 0
        point_features_for_statistic = point_features.clone()
            ...
            for i in range(3):
                ...
                if len(cur_class_features) > 0:
                    ...
                    with torch.no_grad():
                        ...
                        self.object_statistic_features[i, :] = self.object_statistic_features[i, :] + \
                                                               self.object_momentum[i]
        ...
        _, point_features, _, _, _, _, _, _, _ = self.VSA_module(
            xyz=point_coords,
            new_xyz=vote_coords,
            # scores= batch_dict['last_scores'],
        ...
        # batch_dict['l_new_centroids_masked'].append(l_new_centroids_masked)
        # batch_dict['l_sp_tensor_features_selected'].append(l_sp_tensor_features_selected)
        ...
        # point_features = point_features.permute(0,2,1).contiguous().view(num_point, -1)
        # vote_cls_aware = vote_cls_aware.permute(0,2,1).contiguous().view(num_point, -1)
        ...
        # point_reg_preds = self.reg_layers(vote_cls_aware)
